...
async def process_file(input_path: str, out_tsv: str, checkpoint_file: str, genanki_flag: bool = False, dry_run: bool = False, limit: int | None = None, mode: str = "process", max_completion_tokens: int = 4000):
    ...
    logger.info("Starting processing; mode=%s, limit=%s, max_completion_tokens=%d", mode, (limit if limit is not None else "no limit"), max_completion_tokens)
    ...
    to_process = [s for s in sections if (s.get("date_str") or "unknown") not in existing]
    logger.info("Sections to process: %d (skipping %d existing)", len(to_process), len(sections) - len(to_process))
    ...
    for s in tqdm(to_process, desc="Sections"):
        ...
        # Call OpenAI
        try:
            cards = await call_openai_for_section(s.get("content", ""), date_str, max_retries=3, max_completion_tokens=max_completion_tokens)
            # normalize
            for c in cards:
                if "tags" not in c:
            ...
            try:
                ...
                logger.warning("Failed to write section output for %s: %s", date_str, _e)
...
if __name__ == '__main__':
    ...
    p.add_argument("--limit", type=int, default=None, help="Optional limit of new lessons to process")
    p.add_argument("--mode", choices=["process", "assemble"], default="process", help="Operation mode: 'process' calls OpenAI and writes per-section outputs; 'assemble' builds the final TSV/APKG from outputs/")
    p.add_argument("--max-tokens", type=int, default=4000, help="Max completion tokens to request from the model (default: 4000)")
    args = p.parse_args()
    asyncio.run(process_file(args.input, args.out, args.checkpoint, args.genanki, dry_run=args.dry_run, limit=args.limit, mode=args.mode, max_completion_tokens=args.max_tokens))
